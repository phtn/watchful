...
import type { VirtualBankrollSnapshot } from '@/src/lib/virtual-bankroll'
...
interface OriginalsWorkspaceProps {
  ...
  vrBankProps: VirtualBankrollCardProps
...
export function OriginalsWorkspace({
  ...
  return (
    <>
      ...
      <section className='relative bg-[#232830] p-4 h-[60lvh]'>
        <img
        ...
        <div className='relative z-10 flex items-center justify-between gap-3'>
          ...
          <button
            ...
            className='rounded-full border border-slate-200/80 bg-white px-3 py-1 text-xs font-semibold uppercase tracking-[0.22em] text-slate-800 transition hover:border-slate-300 hover:text-slate-950'>
            ...
            <div
              ...
              className={`rounded-full border px-3 py-1.5 text-xs font-semibold uppercase tracking-[0.18em] ${
                status.site === site.key
                  ...
                  : site.key === 'bet88'
                    ? 'bg-[#fcd806] border-transparent text-slate-900'
        ...
        <div className='relative z-10 mt-4 grid grid-cols-2 gap-3'>
          <ProviderMetric
            ...
            tone='bg-black/50 backdrop-blur-sm text-[#46D266]'
          />
          <ProviderMetric
            label='Stake'
        ...
        <div className='relative z-10 mt-4 rounded-md bg-black/50 backdrop-blur-sm text-[#46D266] p-4'>
          <p className='text-[0.64rem] uppercase tracking-[0.26em] text-white'>Dev server relay</p>
          <p className='mt-2 text-sm text-white'>
            Mirroring tracked rounds to <code className='rounded'>http://localhost:{devServerPort}/api/results</code>
          </p>
          ...
          {settingsVisible ? (
            ...
            <p className='mt-4 text-sm font-semibold text-[#46D266]'>Port {devServerPort}</p>
          )}
        </div>
      </section>
    </>
